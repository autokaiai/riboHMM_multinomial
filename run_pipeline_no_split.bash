...
# DIFFERENCE: This pipeline evaluates on the training set (no train/test split).
...
# --- Function to run a single pipeline (no split) ---
run_pipeline_no_split() {
    TOP_N=$1
    METRIC=$2
    ID_TAG="top${TOP_N}_${METRIC}_nosplit"

    echo "--- Starting no-split pipeline for: ${ID_TAG} with random state ${RANDOM_STATE} ---"
    ...
    # 2. Train Model (using full processed dataset)
    python train.py \
    ...
    # 3. Validate Model on Training Set (same dataset used for training)
    python validate.py \
        --input_npz "${PROCESSED_NPZ}" \
    ...
    echo "--- Finished no-split pipeline for: ${ID_TAG} ---"
}

# --- Run all combinations in parallel ---

# Configurations for TPM
run_pipeline_no_split 500 tpm &
run_pipeline_no_split 1000 tpm &
run_pipeline_no_split 2000 tpm &

# Configurations for raw counts (RPM)
run_pipeline_no_split 500 raw &
run_pipeline_no_split 1000 raw &
run_pipeline_no_split 2000 raw &

# Wait for all parallel jobs to complete
wait
echo "All no-split hyperparameter test runs have completed." 